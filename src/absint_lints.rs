...
// ============================================================================
// 3. Unused Hot Potato (CFG-aware dataflow analysis)
// ============================================================================

const UNUSED_HOT_POTATO_DIAG: DiagnosticInfo = custom(
    LINT_WARNING_PREFIX,
    Severity::Warning,
    CLIPPY_CATEGORY,
    4, // unused_hot_potato
    "hot potato created but not consumed",
);

pub static UNUSED_HOT_POTATO: LintDescriptor = LintDescriptor {
    name: "unused_hot_potato",
    category: LintCategory::Security,
    description: "Hot potato (no-ability struct) created but not consumed - must be returned, passed to function, or unpacked (type-based CFG-aware, requires --mode full --preview)",
    group: RuleGroup::Preview,
    fix: FixDescriptor::none(),
    analysis: AnalysisKind::TypeBasedCFG,
};

pub struct UnusedHotPotatoVerifier;

pub struct UnusedHotPotatoVerifierAI<'a> {
    info: &'a TypingProgramInfo,
    context: &'a CFGContext<'a>,
}

/// Abstract value for hot potato tracking.
///
/// Lattice: NotHotPotato < FreshHotPotato
/// Join is pessimistic: if any path has an unconsumed hot potato, report it.
#[derive(Copy, Clone, Debug, PartialEq, Eq, Default)]
pub enum HotPotatoValue {
    /// Variable holds a fresh hot potato (from Pack)
    FreshHotPotato(Loc),
    /// Not a hot potato (or was consumed/returned)
    #[default]
    NotHotPotato,
}

pub struct HotPotatoExecutionContext {
    diags: CompilerDiagnostics,
}

#[derive(Clone, Debug)]
pub struct HotPotatoState {
    locals: BTreeMap<Var, LocalState<HotPotatoValue>>,
    /// Track hot potatoes that were returned (not an error)
    returned_hot_potatoes: BTreeSet<Loc>,
    /// Track hot potatoes that were consumed (passed to function or unpacked)
    consumed_hot_potatoes: BTreeSet<Loc>,
}

impl SimpleAbsIntConstructor for UnusedHotPotatoVerifier {
    type AI<'a> = UnusedHotPotatoVerifierAI<'a>;

    fn new<'a>(
        context: &'a CFGContext<'a>,
        _cfg: &ImmForwardCFG,
        _init_state: &mut HotPotatoState,
    ) -> Option<Self::AI<'a>> {
        // Skip test functions
        if context.attributes.is_test_or_test_only() {
            return None;
        ...
        Some(UnusedHotPotatoVerifierAI {
            info: context.info,
            context,
        })
    }
...
impl SimpleAbsInt for UnusedHotPotatoVerifierAI<'_> {
    type State = HotPotatoState;
    type ExecutionContext = HotPotatoExecutionContext;
    ...
    fn finish(
        ...
        // Check all final states for unconsumed hot potatoes
        for (_label, state) in final_states {
            for (var, local_state) in &state.locals {
                if let LocalState::Available(_loc, HotPotatoValue::FreshHotPotato(create_loc)) =
                    local_state
                {
                    // Check if this hot potato was returned or consumed
                    if state.returned_hot_potatoes.contains(create_loc) {
                        continue;
                    }
                    if state.consumed_hot_potatoes.contains(create_loc) {
                        continue;
                    }

                    // Only report for root package
                    if !self.context.is_root_package || self.context.is_dependency {
                        continue;
                    }
                    ...
                    let help = "Either return the hot potato, pass it to a function that consumes it, or unpack it.";
                    result_diags.add(diag!(
                        UNUSED_HOT_POTATO_DIAG,
                        (*create_loc, msg),
                        (*create_loc, help),
                    ));
                }
    ...
    fn command_custom(
        ...
        match &cmd.value {
            C::Return { exp, .. } => {
                ...
                // Mark any hot potatoes being returned
                for v in values {
                    if let HotPotatoValue::FreshHotPotato(loc) = v {
                        state.returned_hot_potatoes.insert(loc);
                    }
                }
                true
            }
    ...
    fn exp_custom(
        ...
        state: &mut Self::State,
        e: &Exp,
    ) -> Option<Vec<HotPotatoValue>> {
        use UnannotatedExp_ as E;

        match &e.exp.value {
            // Detect Pack of hot potato types
            E::Pack(struct_name, _type_args, fields) => {
                // Recurse into field expressions first
                for (_, _, field_exp) in fields {
                    self.exp(context, state, field_exp);
                }

                // Check if this struct is a hot potato (no abilities)
                if self.is_hot_potato_struct(struct_name) {
                    // Skip empty structs (0 fields) - these are witness/marker types
                    if fields.is_empty() {
                        return Some(vec![HotPotatoValue::NotHotPotato]);
                    }
                    return Some(vec![HotPotatoValue::FreshHotPotato(e.exp.loc)]);
                }

                Some(vec![HotPotatoValue::NotHotPotato])
            }
            _ => None,
        }
    }
    ...
    fn call_custom(
        ...
        _return_ty: &Type,
        _f: &ModuleCall,
        args: Vec<HotPotatoValue>,
    ) -> Option<Vec<HotPotatoValue>> {
        // Any hot potato passed as an argument is consumed
        for arg_value in args {
            if let HotPotatoValue::FreshHotPotato(loc) = arg_value {
                state.consumed_hot_potatoes.insert(loc);
            }
        }

        // We don't track hot potatoes returned from function calls (deferred decision)
        None
    }
    ...
    fn lvalue_custom(
        ...
        state: &mut Self::State,
        l: &LValue,
        value: &HotPotatoValue,
    ) -> bool {
        use LValue_ as L;

        match &l.value {
            // Unpack destructs the hot potato - counts as consumption
            L::Unpack(_, _, fields) | L::UnpackVariant(_, _, _, _, _, fields) => {
                if let HotPotatoValue::FreshHotPotato(loc) = value {
                    state.consumed_hot_potatoes.insert(*loc);
                }

                // Process nested lvalues with default values
                for (_, nested_l) in fields {
                    self.lvalue(
                        _context,
                        state,
                        nested_l,
                        HotPotatoValue::NotHotPotato,
                    );
                }
                true
            }
...
impl UnusedHotPotatoVerifierAI<'_> {
        ...
        for (_mident, minfo) in self.info.modules.key_cloned_iter() {
            if let Some(sdef) = minfo.structs.get_(&name_symbol) {
                ...
                let is_hot_potato = !abilities.has_ability_(Ability_::Key)
                    ...
                    && !abilities.has_ability_(Ability_::Copy)
                    && !abilities.has_ability_(Ability_::Drop);
                return is_hot_potato;
            }
        }

        // If we can't find the struct definition, be conservative and don't flag
        false
    }
}

impl SimpleDomain for HotPotatoState {
    type Value = HotPotatoValue;

    fn new(_context: &CFGContext, locals: BTreeMap<Var, LocalState<Self::Value>>) -> Self {
        HotPotatoState {
            locals,
            returned_hot_potatoes: BTreeSet::new(),
            consumed_hot_potatoes: BTreeSet::new(),
        }
    }
    ...
    fn join_value(v1: &Self::Value, v2: &Self::Value) -> Self::Value {
        ...
        match (v1, v2) {
            ...
            (FreshHotPotato(loc), _) | (_, FreshHotPotato(loc)) => FreshHotPotato(*loc),
            (NotHotPotato, NotHotPotato) => NotHotPotato,
        }
    }

    fn join_impl(&mut self, other: &Self, _result: &mut JoinResult) {
        // Merge returned and consumed sets
        self.returned_hot_potatoes
            .extend(other.returned_hot_potatoes.iter().copied());
        self.consumed_hot_potatoes
            .extend(other.consumed_hot_potatoes.iter().copied());
    }
}

impl SimpleExecutionContext for HotPotatoExecutionContext {
    fn add_diag(&mut self, diag: CompilerDiagnostic) {
        self.diags.add(diag)
    }
}
...
static DESCRIPTORS: &[&LintDescriptor] = &[&PHANTOM_CAPABILITY, &UNCHECKED_DIVISION_V2, &UNUSED_HOT_POTATO];
...
pub fn create_visitors(preview: bool) -> Vec<Box<dyn AbstractInterpreterVisitor>> {
    ...
    vec![
        ...
        Box::new(UnusedHotPotatoVerifier) as Box<dyn AbstractInterpreterVisitor>,
    ]
}
