...
| `capability_destroyed_unused` | preview | absint | security | type-based-cfg | `--mode full --preview` | Capability consumed without validation - authorization may be missing (CFG-aware, requires --mode full --preview) |
...
| `droppable_hot_potato_v2` | stable | semantic | security | type-based | `--mode full` | Struct has only `drop` ability - likely a broken hot potato (type-based, zero FP) |
...
| `redundant_self_import` | stable | syntactic | style | syntactic | `fast` | Avoid `use pkg::mod::{Self};`; prefer `use pkg::mod;` |
...
| `self_transfer` | stable | semantic | suspicious | type-based | `--mode full` | [Sui Linter] Transferring object to self - consider returning instead (from sui_mode::linters) |
| `share_owned` | stable | semantic | suspicious | type-based | `--mode full` | [Sui Linter] Possible owned object share (from sui_mode::linters) |
| `share_owned_authority` | stable | semantic | security | type-based | `--mode full` | Sharing key+store object makes it publicly accessible - dangerous for authority objects (type-based) |
...
| `tainted_recipient` | experimental | absint | security | type-based-cfg | `--mode full --experimental` | Asset transferred to address from untrusted source without validation (CFG-aware, requires --mode full --experimental) |
...
| `unused_return_value` | stable | semantic | security | type-based | `--mode full` | Important return value is ignored, may indicate bug (type-based) |
